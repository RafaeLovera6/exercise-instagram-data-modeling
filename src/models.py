...
class Followers(Base):
    ...
    id = Column(Integer, primary_key=True)
    user_id = Column(String(120), ForeignKey('user.id'), nullable=False)
    follower_id = Column(String(120), ForeignKey('user.id'), nullable=False)

class Post(Base):
    __tablename__ = 'post'
    id = Column(Integer, primary_key=True)
    user_id = Column(String(120), ForeignKey('user.id'), nullable=False)
    username = Column(String(120), ForeignKey('user.username'), nullable=False)
...
class Hashtag_Comment(Base):
    ...
    hashtag_text = Column(String(120), nullable=False)
